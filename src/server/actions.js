export async function seedItalianPhrases(args, context) {
    const existingPhrases = await context.entities.Phrase.findMany({});
    ...
    let italian = await context.entities.Language.findUnique({
        where: {
            name: "Italian"
        }
    });
    let english = await context.entities.Language.findUnique({
        where: {
            name: "English"
        }
    });
    if (!italian) {
        italian = await context.entities.Language.create({
            data: {
                name: "Italian",
                emoji: "🇮🇹"
            }
    ...
    if (!english) {
        english = await context.entities.Language.create({
            data: {
                name: "English",
                emoji: "🇬🇧"
            }
